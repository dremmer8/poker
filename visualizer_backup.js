...
class PokerVisualizer {
    constructor() {
        ...
        this.biddingAnnouncementShown = false;
    ...
    updateGameData(gameData) {
        try {
            ...
            let previousRound = 0;
            if (this.currentGame) {
                previousRound = this.currentGame.currentRound || 0;
            }
            ...
            const newRound = this.currentGame.currentRound || 0;
            if (newRound > previousRound && previousRound > 0) {
                this.showNextRoundAnnouncement(previousRound);
            }
    ...
    updateHeader() {
        // Update round number - try different possible properties
        const roundElement = document.getElementById('currentRound');
        if (roundElement && this.currentGame) {
            let roundNumber = this.currentGame.currentRound || 
                             this.currentGame.round || 
                             this.currentGame.roundNumber ||
                             (this.roundResults ? this.roundResults.length + 1 : 1);
        ...
        if (cardsElement && this.currentGame) {
            let cardsPerHand = this.currentGame.cardsPerHand || 
                              this.currentGame.cardsInHand ||
                              this.currentGame.cardsPerPlayer ||
                              0;
            cardsElement.textContent = cardsPerHand;
        }
    ...
    checkBiddingComplete() {
        ...
        const allBidsPlaced = this.players.every(player => {
            const bid = this.getPlayerBid(player);
            return bid !== null && bid !== undefined;
        });
        
        // Check if we're in the tricks phase (bidding is complete)
        const isInTricksPhase = this.currentGame.currentPhase === 'tricks' || 
                               (this.currentGame.currentRoundData && this.currentGame.currentRoundData.phase === 'tricks');
        
        if (allBidsPlaced && isInTricksPhase && !this.biddingAnnouncementShown) {
            this.showBiddingCompleteAnnouncement();
            this.biddingAnnouncementShown = true;
        }
    }
    
    showBiddingCompleteAnnouncement() {
        const overlay = document.getElementById('biddingCompleteOverlay');
        if (!overlay) return;
        
        // Populate bidding summary
        const biddingSummary = document.getElementById('biddingSummary');
        if (biddingSummary) {
            const totalBids = this.calculateTotalBids();
            const totalPossibleTricks = this.currentGame.cardsPerHand || 0;
            const totalBiddingCap = this.players.length * totalPossibleTricks;
            
            biddingSummary.innerHTML = `
                <h3>Сводка ставок</h3>
                <div class="bidding-stats">
                    ...
                    <div class="bidding-stat">
                        <div class="value">${totalBiddingCap}</div>
    ...
    showNextRoundAnnouncement(previousRound) {
        const overlay = document.getElementById('nextRoundOverlay');
        if (!overlay) return;
        
        // Get the completed round data
        const completedRound = this.roundResults.find(r => r.round === previousRound);
        if (!completedRound) return;
        
        // Populate round summary
        const roundSummary = document.getElementById('roundSummary');
        if (roundSummary) {
            const stageInfo = this.getStageInfoForRound(previousRound);
            roundSummary.innerHTML = `
                <h3>Раунд ${previousRound} завершен</h3>
                <div class="round-info">
                    <div class="round-info-item">
                        <div class="value">${completedRound.cardsPerHand}</div>
        ...
        // Populate score changes
        const scoreChanges = document.getElementById('scoreChanges');
        if (scoreChanges && completedRound.results) {
            scoreChanges.innerHTML = `
                <h3>Изменения очков</h3>
                <div class="score-changes-grid">
                    ${completedRound.results.map(result => `
                        <div class="score-change-item">
                            <div class="score-change-player">${result.player}</div>
                            <div class="score-change-details">
                                <div class="score-change-bid">Заказ: ${result.bid}</div>
                                <div class="score-change-tricks">Взятки: ${result.tricks}</div>
                            </div>
                            <div class="score-change-points ${result.points >= 0 ? 'positive' : 'negative'}">
        ...
        // Update subtitle with round number
        const subtitle = overlay.querySelector('.announcement-subtitle');
        if (subtitle) {
            subtitle.textContent = `Раунд ${this.currentGame.currentRound} начинается...`;
        }
